module Jekyll  
  class UltravioletTag < Liquid::Block
      ...
      super
        ...
        if defined?(markup) && markup != ''
          markup.split.each do |opt|
            ...
            case value
            when /vcl/
              # puts "START LOADING VCL!"
              # puts "File: #{File.expand_path('assets/files/source.vcl.syntax')}"
              # processor = Textpow::RecordingProcessor.new
              # Textpow::SyntaxNode.load(File.expand_path('assets/files/source.vcl.syntax'), Jekyll.root )
              # Textpow.syntax(File.expand_path('assets/files/source.vcl.syntax'))
              # processor = Textpow::DebugProcessor.new
              # puts "END LOADING VCL!"
            end
    ...
    def render(context)
      ...
      # this processor is needed for rendering Uv
      # processor = Textpow::RecordingProcessor.new
      # output = Uv.parse(code, "xhtml", "shell", false, "eiffel")
